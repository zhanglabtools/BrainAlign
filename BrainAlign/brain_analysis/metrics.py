...
from BrainAlign.brain_analysis.typehint import RandomState
from BrainAlign.brain_analysis.utils import get_rs
...
if __name__ == '__main__':
    x = np.concatenate([np.random.rand(100, 5), np.ones((100, 5))], axis=0)
    y = np.concatenate([np.zeros((120, 1)), np.ones((80, 1))], axis=0)
    score = seurat_alignment_score(x, y, neighbor_frac=0.1)
    print(score)