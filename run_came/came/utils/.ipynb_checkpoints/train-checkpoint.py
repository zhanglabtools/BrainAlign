...
class Trainer(BaseTrainer):
    ...
    def __init__(self,
        ...
        self.all_labels = torch.concat((train_labels, test_labels))
        ...
        self.classes_test = infer_classes(detach2numpy(self.test_labels))

        print('self.classes', self.classes_test)
        print('len of self.classes', len(self.classes_test))

        self.classes_cluster = infer_classes(detach2numpy(torch.concat((self.train_labels, self.test_labels))))
        self.train_classes = [self.classes_cluster[x] for x in range(len(self.classes))]
        self.test_classes = [self.classes_cluster[x] for x in range(len(self.classes_test))]
        # print('self.classes_cluster', self.classes_cluster)
        # print(type(self.classes_cluster))
    ...
    def set_class_weights(self, class_weights=None, test_class_weights = None, foo=np.sqrt, n_add=0):
        if class_weights is None:
            self.class_weights = make_class_weights(
                self.train_labels, foo=foo, n_add=n_add)
        else:
            self.class_weights = Tensor(class_weights)
        if test_class_weights is None:
            self.test_class_weights = make_class_weights(
                self.test_labels, foo=foo, n_add=n_add)
        else:
            self.test_class_weights = Tensor(test_class_weights)
    ...
    def train(self, n_epochs=350,
        ...
        test_labels_1hot = onehot_encode(
            self.test_labels, self.test_classes,
            sparse_output=False, astensor=True).to(device)
        ...
        for epoch in range(n_epochs):
            ...
            self._record(**rcd)
            self.log_info(**rcd, print_info=self._cur_epoch % info_stride == 0 or backup)
    ...
    def train_minibatch(self, n_epochs=100,
        ...
        print('train_idx type', type(train_idx))
    ...
    def train_minibatch_cluster(self, n_epochs=100,
        ...
        if use_class_weights:
            class_weights = to_device(self.class_weights, device)
            test_class_weights = to_device(self.test_class_weights, device)
        else:
            class_weights = None
        ...
        train_dataloader = dgl.dataloading.DataLoader(
            # The following arguments are specific to NodeDataLoader.
            g,
            {'cell': torch.concat((train_idx, test_idx))},  # The node IDs to iterate over in minibatches
            sampler,
            device=device,  # Put the sampled MFGs on CPU or GPU # The following arguments are inherited from PyTorch DataLoader.
            batch_size=batch_size,
            shuffle=True,
        ...
        test_dataloader_shuffled = dgl.dataloading.DataLoader(
            g,
            {'cell': test_idx},
            sampler,
            device=device,
        ...
        for epoch in range(n_epochs):
            ...
            for input_nodes, output_nodes, mfgs in tqdm.tqdm(train_dataloader):
                ...
                # out_train_labels
                out_train_lbs1hot = onehot_encode(
                    _labels, classes=self.classes_cluster, astensor=True).to(device)
                _labels = to_device(_labels, device)
            ...
            ###################
            # for input_nodes, output_nodes, mfgs in tqdm.tqdm(test_dataloader_shuffled):
            #     self.optimizer.zero_grad()
            #     _labels = mfgs[-1].dstdata['label'][cat_class]
            #     _feat_dict = to_device(idx_hetero(self.feat_dict, input_nodes), device)
            #     mfgs = to_device(mfgs, device)
            ...
            #     logits = model(_feat_dict, mfgs, **other_inputs)[cat_class]
            #     # out_test_labels
            #     out_test_lbs1hot = onehot_encode(
            #         _labels, classes=self.classes_cluster, astensor=True).to(device)
            #     _labels = to_device(_labels, device)
            #     # loss = ce_loss_with_rdrop(
            #     #     logits, logits2, labels=_labels,
